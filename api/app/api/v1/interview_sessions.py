...
def _review_to_out(review: InterviewReview) -> ReviewOut:
    ...
    return ReviewOut(
        ...
        dimensions=parse(review.dimensions_json),
        weak_points=parse(review.weak_points_json),
        review_points=parse(review.review_points_json),
        knowledge_points=parse(review.knowledge_points_json),
...
def generate_review(
    ...
    by_id = {
        int(lab["question_id"]): lab
        for lab in result.get("question_labels") or []
        if isinstance(lab, dict) and lab.get("question_id") is not None
    }
    for q in questions:
        meta = by_id.get(q.id)
        if meta:
            ...
            if meta.get("knowledge_point"):
                q.knowledge_point = meta["knowledge_point"]
            q.source = "AGENT"
    ...
    review = InterviewReview(
        ...
        dimensions_json=json.dumps(result.get("dimensions") or [], ensure_ascii=False),
        weak_points_json=json.dumps(result.get("weak_points") or [], ensure_ascii=False),
        review_points_json=json.dumps(result.get("review_points") or [], ensure_ascii=False),
        knowledge_points_json=json.dumps([], ensure_ascii=False),
        review_advice=None,
    )
    db.add(review)
    db.commit()
